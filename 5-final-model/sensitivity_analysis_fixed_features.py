"""
sensitivity_analysis_fixed_features.py
========================================
Sensitivity analysis with FIXED FEATURES (no RFE).

For each combination of HORIZONTE_DIAS and ALVO:
  1. Re-runs extractingLabeling.py logic to regenerate labeled CSVs.
  2. Runs the XGBoost 3-class model with fixed features and captures metrics.
  3. Saves all results to a summary CSV in 5-final-model/.

Run from the PROJECT ROOT:
    python 5-final-model/sensitivity_analysis_fixed_features.py
"""
...
OUTPUT_DIR = SCRIPT_DIR  # 5-final-model/
OUTPUT_CSV = os.path.join(OUTPUT_DIR, 'sensitivity_results_fixed_features.csv')

# FIXED FEATURES (no RFE) — match xgboost-model-3-classes-fixed-features.py
SELECTED_FEATURES = [
    'DiasParaEleicaoBR',
    'DiasParaEleicaoUSA',
    'Selic_Delta_21d',
...
def run_labeling(horizonte_dias: int, alvo: float) -> dict:
    ...
    for ativo in TICKERS:
        ...
        try:
            ...
            print(f"OK | Dist: {dict(df_final['Alvo'].value_counts())}")
...
# ============================================================================
# STEP 2 — XGBoost 3-class model with FIXED FEATURES (returns metrics dict)
# ============================================================================

def run_xgboost_model(df_fed: pd.DataFrame, df_selic: pd.DataFrame) -> dict:
    """
    Trains and evaluates the XGBoost 3-class model with FIXED FEATURES.
    ...
    for arquivo in arquivos:
        try:
            ...
            # --- FEATURE ENGINEERING (match fixed-features version) ---
            for lag in [5, 21, 63]: 
    ...
    # Filter to the FIXED feature set
    ...
    for cls in classes_nomes:
        r = report[cls]
        print(f"  {cls:8s}  Prec={r['precision']:.4f}  Rec={r['recall']:.4f}  F1={r['f1-score']:.4f}")
...
def main():
    ...
    for horizonte in horizonte_values:
        for alvo in THRESHOLDS:
            ...
            try:
                ...
                metrics = {
                    'accuracy': np.nan,
                    'venda_precision': np.nan,
                    'venda_recall': np.nan,
                    'venda_f1': np.nan,
                    'neutro_precision': np.nan,
                    'neutro_recall': np.nan,
                    'neutro_f1': np.nan,
                    'compra_precision': np.nan,
                    'compra_recall': np.nan,
                    'compra_f1': np.nan,
                    'macro_avg_f1': np.nan,
                    'weighted_avg_f1': np.nan,
                }
    ...
    print(f"\n\n{'='*70}")
    print(f"  ANÁLISE DE SENSIBILIDADE (FIXED FEATURES) CONCLUÍDA")
    print(f"  Resultados salvos em: {OUTPUT_CSV}")
    print(f"  Total de iterações: {len(all_results)}")
    print(f"{'='*70}\n")
    print(df_results[['horizonte_dias', 'alvo_retorno', 'accuracy', 'macro_avg_f1', 'EV_Compra', 'EV_Venda']].to_string(index=False))
